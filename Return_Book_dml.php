...
// Data functions (insert, update, delete, form) for table Return_Book

// This script and data application were generated by AppGini 5.70
// Download AppGini for free from https://bigprof.com/appgini/download/

function Return_Book_insert(){
	global $Translation;
	...
	// hook: Return_Book_before_insert
	if(function_exists('Return_Book_before_insert')){
		$args=array();
		if(!Return_Book_before_insert($data, getMemberInfo(), $args)){ return false; }
	}
	...
	// hook: Return_Book_after_insert
	if(function_exists('Return_Book_after_insert')){
		$res = sql("select * from `Return_Book` where `id`='" . makeSafe($recID, false) . "' limit 1", $eo);
		if($row = db_fetch_assoc($res)){
			$data = array_map('makeSafe', $row);
		}
		$data['selectedID'] = makeSafe($recID, false);
		$args=array();
		if(!Return_Book_after_insert($data, getMemberInfo(), $args)){ return $recID; }
	}

	// mm: save ownership data
	set_record_owner('Return_Book', $recID, getLoggedMemberID());

	return $recID;
}

function Return_Book_delete($selected_id, $AllowDeleteOfParents=false, $skipChecks=false){
	...
	$arrPerm=getTablePermissions('Return_Book');
	$ownerGroupID=sqlValue("select groupID from membership_userrecords where tableName='Return_Book' and pkValue='$selected_id'");
	$ownerMemberID=sqlValue("select lcase(memberID) from membership_userrecords where tableName='Return_Book' and pkValue='$selected_id'");
	...
	// hook: Return_Book_before_delete
	if(function_exists('Return_Book_before_delete')){
		$args=array();
		if(!Return_Book_before_delete($selected_id, $skipChecks, getMemberInfo(), $args))
			return $Translation['Couldn\'t delete this record'];
	}

	sql("delete from `Return_Book` where `id`='$selected_id'", $eo);

	// hook: Return_Book_after_delete
	if(function_exists('Return_Book_after_delete')){
		$args=array();
		Return_Book_after_delete($selected_id, getMemberInfo(), $args);
	}

	// mm: delete ownership data
	sql("delete from membership_userrecords where tableName='Return_Book' and pkValue='$selected_id'", $eo);
}

function Return_Book_update($selected_id){
	global $Translation;

	// mm: can member edit record?
	$arrPerm=getTablePermissions('Return_Book');
	$ownerGroupID=sqlValue("select groupID from membership_userrecords where tableName='Return_Book' and pkValue='".makeSafe($selected_id)."'");
	$ownerMemberID=sqlValue("select lcase(memberID) from membership_userrecords where tableName='Return_Book' and pkValue='".makeSafe($selected_id)."'");
	...
	// hook: Return_Book_before_update
	if(function_exists('Return_Book_before_update')){
		$args=array();
		if(!Return_Book_before_update($data, getMemberInfo(), $args)){ return false; }
	}
	...
	// hook: Return_Book_after_update
	if(function_exists('Return_Book_after_update')){
		$res = sql("SELECT * FROM `Return_Book` WHERE `id`='{$data['selectedID']}' LIMIT 1", $eo);
		if($row = db_fetch_assoc($res)){
			$data = array_map('makeSafe', $row);
		}
		$data['selectedID'] = $data['id'];
		$args = array();
		if(!Return_Book_after_update($data, getMemberInfo(), $args)){ return; }
	}

	// mm: update ownership data
	sql("update membership_userrecords set dateUpdated='".time()."' where tableName='Return_Book' and pkValue='".makeSafe($selected_id)."'", $eo);

}

function Return_Book_form($selected_id = '', $AllowUpdate = 1, $AllowInsert = 1, $AllowDelete = 1, $ShowCancel = 0, $TemplateDV = '', $TemplateDVP = ''){
	...
	if(is_file(dirname(__FILE__).'/hooks/Return_Book.Status.csv')){
		$Status_data = addslashes(implode('', @file(dirname(__FILE__).'/hooks/Return_Book.Status.csv')));
	...
	if($selected_id){
		...
		$res = sql("select * from `Return_Book` where `id`='".makeSafe($selected_id)."'", $eo);
		if(!($row = db_fetch_array($res))){
			return error_message($Translation['No records found'], 'Return_Book_view.php', false);
		}
			...
			$j("#Book_Number-container__RAND__").select2({
				...
				initSelection: function(e, c){
					$j.ajax({
						url: 'ajax_combo.php',
						dataType: 'json',
						data: { id: AppGini.current_Book_Number__RAND__.value, t: 'Return_Book', f: 'Book_Number' },
				...
				ajax: {
					...
					dataType: 'json',
					cache: true,
					data: function(term, page){ return { s: term, p: page, t: 'Return_Book', f: 'Book_Number' }; },
			...
			$j("#Member-container__RAND__").select2({
				...
				initSelection: function(e, c){
					$j.ajax({
						url: 'ajax_combo.php',
						dataType: 'json',
						data: { id: AppGini.current_Member__RAND__.value, t: 'Return_Book', f: 'Member' },
						success: function(resp){
							c({
								id: resp.results[0].id,
								text: resp.results[0].text
				...
				ajax: {
					...
					dataType: 'json',
					cache: true,
					data: function(term, page){ return { s: term, p: page, t: 'Return_Book', f: 'Member' }; },
					results: function(resp, page){ return resp; }
				},
				escapeMarkup: function(str){ return str; }
			}).on('change', function(e){
	...
	if($dvprint){
		$template_file = is_file("./{$TemplateDVP}") ? "./{$TemplateDVP}" : './templates/Return_Book_templateDVP.html';
		$templateCode = @file_get_contents($template_file);
	}else{
		$template_file = is_file("./{$TemplateDV}") ? "./{$TemplateDV}" : './templates/Return_Book_templateDV.html';
		$templateCode = @file_get_contents($template_file);
	}
	...
	// process buttons
	if($AllowInsert){
		if(!$selected_id) $templateCode = str_replace('<%%INSERT_BUTTON%%>', '<button type="submit" class="btn btn-success" id="insert" name="insert_x" value="1" onclick="return Return_Book_validateData();"><i class="glyphicon glyphicon-plus-sign"></i> ' . $Translation['Save New'] . '</button>', $templateCode);
		$templateCode = str_replace('<%%INSERT_BUTTON%%>', '<button type="submit" class="btn btn-default" id="insert" name="insert_x" value="1" onclick="return Return_Book_validateData();"><i class="glyphicon glyphicon-plus-sign"></i> ' . $Translation['Save As Copy'] . '</button>', $templateCode);
	}else{
	...
	if($selected_id){
		...
		if($AllowUpdate){
			$templateCode = str_replace('<%%UPDATE_BUTTON%%>', '<button type="submit" class="btn btn-success btn-lg" id="update" name="update_x" value="1" onclick="return Return_Book_validateData();" title="' . html_attr($Translation['Save Changes']) . '"><i class="glyphicon glyphicon-ok"></i> ' . $Translation['Save Changes'] . '</button>', $templateCode);
		}else{
	...
	$templateCode .= "\t\t\$j.ajax({\n";
	if($dvprint){
		$templateCode .= "\t\t\turl: 'Return_Book_autofill.php?rnd1=$rnd1&mfk=Book_Number&id=' + encodeURIComponent('".addslashes($row['Book_Number'])."'),\n";
		$templateCode .= "\t\t\tcontentType: 'application/x-www-form-urlencoded; charset=" . datalist_db_encoding . "', type: 'GET'\n";
	}else{
		$templateCode .= "\t\t\turl: 'Return_Book_autofill.php?rnd1=$rnd1&mfk=Book_Number&id=' + encodeURIComponent(AppGini.current_Book_Number{$rnd1}.value),\n";
	...
	$templateCode .= "\t\t\$j.ajax({\n";
	if($dvprint){
		$templateCode .= "\t\t\turl: 'Return_Book_autofill.php?rnd1=$rnd1&mfk=Member&id=' + encodeURIComponent('".addslashes($row['Member'])."'),\n";
		$templateCode .= "\t\t\tcontentType: 'application/x-www-form-urlencoded; charset=" . datalist_db_encoding . "', type: 'GET'\n";
	}else{
		$templateCode .= "\t\t\turl: 'Return_Book_autofill.php?rnd1=$rnd1&mfk=Member&id=' + encodeURIComponent(AppGini.current_Member{$rnd1}.value),\n";
	...
	$rdata = $jdata = get_defaults('Return_Book');
	if($selected_id){
		$jdata = get_joined_record('Return_Book', $selected_id);
		if($jdata === false) $jdata = get_defaults('Return_Book');
		$rdata = $row;
	}
	$cache_data = array(
	...
	$templateCode .= loadView('Return_Book-ajax-cache', $cache_data);

	// hook: Return_Book_dv
	if(function_exists('Return_Book_dv')){
		$args=array();
		Return_Book_dv(($selected_id ? $selected_id : FALSE), getMemberInfo(), $templateCode, $args);
	}
