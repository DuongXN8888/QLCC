...
function Ma_insert(){
	...
	$arrPerm=getTablePermissions('Ma');
	if(!$arrPerm[1]){
		return false;
	}
	...
	if(function_exists('Ma_before_insert')){
		$args=array();
		if(!Ma_before_insert($data, getMemberInfo(), $args)){ return false; }
	}

	$o = array('silentErrors' => true);
	sql('insert into `Ma` set       `Type`=' . (($data['Type'] !== '' && $data['Type'] !== NULL) ? "'{$data['Type']}'" : 'NULL') . ', `Name`=' . (($data['Name'] !== '' && $data['Name'] !== NULL) ? "'{$data['Name']}'" : 'NULL') . ', `Date_Of_Receipt`=' . (($data['Date_Of_Receipt'] !== '' && $data['Date_Of_Receipt'] !== NULL) ? "'{$data['Date_Of_Receipt']}'" : 'NULL') . ', `Date_Published`=' . (($data['Date_Published'] !== '' && $data['Date_Published'] !== NULL) ? "'{$data['Date_Published']}'" : 'NULL') . ', `Pages`=' . (($data['Pages'] !== '' && $data['Pages'] !== NULL) ? "'{$data['Pages']}'" : 'NULL') . ', `Price`=' . (($data['Price'] !== '' && $data['Price'] !== NULL) ? "'{$data['Price']}'" : 'NULL') . ', `Publisher`=' . (($data['Publisher'] !== '' && $data['Publisher'] !== NULL) ? "'{$data['Publisher']}'" : 'NULL'), $o);
	if($o['error']!=''){
		echo $o['error'];
		echo "<a href=\"Ma_view.php?addNew_x=1\">{$Translation['< back']}</a>";
		exit;
	}
	...
	if(function_exists('Ma_after_insert')){
		$res = sql("select * from `Ma` where `id`='" . makeSafe($recID, false) . "' limit 1", $eo);
		if($row = db_fetch_assoc($res)){
			$data = array_map('makeSafe', $row);
		}
		$data['selectedID'] = makeSafe($recID, false);
		$args=array();
		if(!Ma_after_insert($data, getMemberInfo(), $args)){ return $recID; }
	}

	// mm: save ownership data
	set_record_owner('Ma', $recID, getLoggedMemberID());

	return $recID;
}

function Ma_delete($selected_id, $AllowDeleteOfParents=false, $skipChecks=false){
	...
	$arrPerm=getTablePermissions('Ma');
	$ownerGroupID=sqlValue("select groupID from membership_userrecords where tableName='Ma' and pkValue='$selected_id'");
	$ownerMemberID=sqlValue("select lcase(memberID) from membership_userrecords where tableName='Ma' and pkValue='$selected_id'");
	...
	if(function_exists('Ma_before_delete')){
		$args=array();
		if(!Ma_before_delete($selected_id, $skipChecks, getMemberInfo(), $args))
			return $Translation['Couldn\'t delete this record'];
	}

	sql("delete from `Ma` where `id`='$selected_id'", $eo);

	// hook: Magazines_after_delete
	if(function_exists('Ma_after_delete')){
		$args=array();
		Ma_after_delete($selected_id, getMemberInfo(), $args);
	}

	// mm: delete ownership data
	sql("delete from membership_userrecords where tableName='Ma' and pkValue='$selected_id'", $eo);
}

function Ma_update($selected_id){
	global $Translation;

	// mm: can member edit record?
	$arrPerm=getTablePermissions('Ma');
	$ownerGroupID=sqlValue("select groupID from membership_userrecords where tableName='Ma' and pkValue='".makeSafe($selected_id)."'");
	$ownerMemberID=sqlValue("select lcase(memberID) from membership_userrecords where tableName='Ma' and pkValue='".makeSafe($selected_id)."'");
	...
	if(function_exists('Ma_before_update')){
		$args=array();
		if(!Ma_before_update($data, getMemberInfo(), $args)){ return false; }
	}

	$o=array('silentErrors' => true);
	sql('update `Ma` set       `Type`=' . (($data['Type'] !== '' && $data['Type'] !== NULL) ? "'{$data['Type']}'" : 'NULL') . ', `Name`=' . (($data['Name'] !== '' && $data['Name'] !== NULL) ? "'{$data['Name']}'" : 'NULL') . ', `Date_Of_Receipt`=' . (($data['Date_Of_Receipt'] !== '' && $data['Date_Of_Receipt'] !== NULL) ? "'{$data['Date_Of_Receipt']}'" : 'NULL') . ', `Date_Published`=' . (($data['Date_Published'] !== '' && $data['Date_Published'] !== NULL) ? "'{$data['Date_Published']}'" : 'NULL') . ', `Pages`=' . (($data['Pages'] !== '' && $data['Pages'] !== NULL) ? "'{$data['Pages']}'" : 'NULL') . ', `Price`=' . (($data['Price'] !== '' && $data['Price'] !== NULL) ? "'{$data['Price']}'" : 'NULL') . ', `Publisher`=' . (($data['Publisher'] !== '' && $data['Publisher'] !== NULL) ? "'{$data['Publisher']}'" : 'NULL') . " where `id`='".makeSafe($selected_id)."'", $o);
	if($o['error']!=''){
		echo $o['error'];
		echo '<a href="Ma_view.php?SelectedID='.urlencode($selected_id)."\">{$Translation['< back']}</a>";
		exit;
	}


	// hook: Magazines_after_update
	if(function_exists('Ma_after_update')){
		$res = sql("SELECT * FROM `Ma` WHERE `id`='{$data['selectedID']}' LIMIT 1", $eo);
		if($row = db_fetch_assoc($res)){
			$data = array_map('makeSafe', $row);
		}
		$data['selectedID'] = $data['id'];
		$args = array();
		if(!Ma_after_update($data, getMemberInfo(), $args)){ return; }
	}

	// mm: update ownership data
	sql("update membership_userrecords set dateUpdated='".time()."' where tableName='Ma' and pkValue='".makeSafe($selected_id)."'", $eo);

}

function Ma_form($selected_id = '', $AllowUpdate = 1, $AllowInsert = 1, $AllowDelete = 1, $ShowCancel = 0, $TemplateDV = '', $TemplateDVP = ''){
	...
	if($selected_id){
		...
		$res = sql("select * from `Ma` where `id`='".makeSafe($selected_id)."'", $eo);
		if(!($row = db_fetch_array($res))){
			return error_message($Translation['No records found'], 'Ma_view.php', false);
		}
	...
	if($dvprint){
		$template_file = is_file("./{$TemplateDVP}") ? "./{$TemplateDVP}" : './templates/Ma_templateDVP.html';
		$templateCode = @file_get_contents($template_file);
	}else{
		$template_file = is_file("./{$TemplateDV}") ? "./{$TemplateDV}" : './templates/Ma_templateDV.html';
		$templateCode = @file_get_contents($template_file);
	}
	...
	// process buttons
	if($AllowInsert){
		if(!$selected_id) $templateCode = str_replace('<%%INSERT_BUTTON%%>', '<button type="submit" class="btn btn-success" id="insert" name="insert_x" value="1" onclick="return Ma_validateData();"><i class="glyphicon glyphicon-plus-sign"></i> ' . $Translation['Save New'] . '</button>', $templateCode);
		$templateCode = str_replace('<%%INSERT_BUTTON%%>', '<button type="submit" class="btn btn-default" id="insert" name="insert_x" value="1" onclick="return Ma_validateData();"><i class="glyphicon glyphicon-plus-sign"></i> ' . $Translation['Save As Copy'] . '</button>', $templateCode);
	}else{
	...
	if($selected_id){
		...
		if($AllowUpdate){
			$templateCode = str_replace('<%%UPDATE_BUTTON%%>', '<button type="submit" class="btn btn-success btn-lg" id="update" name="update_x" value="1" onclick="return Ma_validateData();" title="' . html_attr($Translation['Save Changes']) . '"><i class="glyphicon glyphicon-ok"></i> ' . $Translation['Save Changes'] . '</button>', $templateCode);
		}else{
	...
	$rdata = $jdata = get_defaults('Ma');
	if($selected_id){
		$jdata = get_joined_record('Ma', $selected_id);
		if($jdata === false) $jdata = get_defaults('Ma');
		$rdata = $row;
	}
	$cache_data = array(
	...
	$templateCode .= loadView('Ma-ajax-cache', $cache_data);

	// hook: Magazines_dv
	if(function_exists('Ma_dv')){
		$args=array();
		Ma_dv(($selected_id ? $selected_id : FALSE), getMemberInfo(), $templateCode, $args);
	}
