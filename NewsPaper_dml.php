...
function NewsPaper_insert(){
	...
	$arrPerm=getTablePermissions('NewsPaper');
	if(!$arrPerm[1]){
		return false;
	}
	...
	$data['Publisher'] = makeSafe($_REQUEST['Publisher']);
		...
		$data['Typess'] = makeSafe($_REQUEST['Typess']);
		if($data['Typess'] == empty_lookup_value){ $data['Typess'] = ''; }
	$data['Pu'] = makeSafe($_REQUEST['Pu']);
		if($data['Pu'] == empty_lookup_value){ $data['Pu'] = ''; }
	if($data['Price'] == '') $data['Price'] = "0.00";

	// hook: NewsPapers_before_insert
	if(function_exists('NewsPaper_before_insert')){
		$args=array();
		if(!NewsPaper_before_insert($data, getMemberInfo(), $args)){ return false; }
	}

	$o = array('silentErrors' => true);
	sql('insert into `NewsPaper` set       `Language`=' . (($data['Language'] !== '' && $data['Language'] !== NULL) ? "'{$data['Language']}'" : 'NULL') . ', `Name`=' . (($data['Name'] !== '' && $data['Name'] !== NULL) ? "'{$data['Name']}'" : 'NULL') . ', `Date_Of_Receipt`=' . (($data['Date_Of_Receipt'] !== '' && $data['Date_Of_Receipt'] !== NULL) ? "'{$data['Date_Of_Receipt']}'" : 'NULL') . ', `Date_Published`=' . (($data['Date_Published'] !== '' && $data['Date_Published'] !== NULL) ? "'{$data['Date_Published']}'" : 'NULL') . ', `Pages`=' . (($data['Pages'] !== '' && $data['Pages'] !== NULL) ? "'{$data['Pages']}'" : 'NULL') . ', `Price`=' . (($data['Price'] !== '' && $data['Price'] !== NULL) ? "'{$data['Price']}'" : 'NULL') . ', `Type`=' . (($data['Type'] !== '' && $data['Type'] !== NULL) ? "'{$data['Type']}'" : 'NULL') . ', `Publisher`=' . (($data['Publisher'] !== '' && $data['Publisher'] !== NULL) ? "'{$data['Publisher']}'" : 'NULL'). ', `Typess`=' . (($data['Typess'] !== '' && $data['Typess'] !== NULL) ? "'{$data['Typess']}'" : 'NULL'). ', `Pu`=' . (($data['Pu'] !== '' && $data['Pu'] !== NULL) ? "'{$data['Pu']}'" : 'NULL'), $o);
	if($o['error']!=''){
		echo $o['error'];
		echo "<a href=\"NewsPaper_view.php?addNew_x=1\">{$Translation['< back']}</a>";
		exit;
	}
	...
	if(function_exists('NewsPaper_after_insert')){
		$res = sql("select * from `NewsPaper` where `id`='" . makeSafe($recID, false) . "' limit 1", $eo);
		if($row = db_fetch_assoc($res)){
			$data = array_map('makeSafe', $row);
		}
		$data['selectedID'] = makeSafe($recID, false);
		$args=array();
		if(!NewsPaper_after_insert($data, getMemberInfo(), $args)){ return $recID; }
	}

	// mm: save ownership data
	set_record_owner('NewsPaper', $recID, getLoggedMemberID());

	return $recID;
}

function NewsPaper_delete($selected_id, $AllowDeleteOfParents=false, $skipChecks=false){
	...
	$arrPerm=getTablePermissions('NewsPaper');
	$ownerGroupID=sqlValue("select groupID from membership_userrecords where tableName='NewsPaper' and pkValue='$selected_id'");
	$ownerMemberID=sqlValue("select lcase(memberID) from membership_userrecords where tableName='NewsPaper' and pkValue='$selected_id'");
	...
	if(function_exists('NewsPaper_before_delete')){
		$args=array();
		if(!NewsPaper_before_delete($selected_id, $skipChecks, getMemberInfo(), $args))
			return $Translation['Couldn\'t delete this record'];
	}

	sql("delete from `NewsPaper` where `id`='$selected_id'", $eo);

	// hook: NewsPapers_after_delete
	if(function_exists('NewsPaper_after_delete')){
		$args=array();
		NewsPaper_after_delete($selected_id, getMemberInfo(), $args);
	}

	// mm: delete ownership data
	sql("delete from membership_userrecords where tableName='NewsPaper' and pkValue='$selected_id'", $eo);
}

function NewsPaper_update($selected_id){
	global $Translation;

	// mm: can member edit record?
	$arrPerm=getTablePermissions('NewsPaper');
	$ownerGroupID=sqlValue("select groupID from membership_userrecords where tableName='NewsPaper' and pkValue='".makeSafe($selected_id)."'");
	$ownerMemberID=sqlValue("select lcase(memberID) from membership_userrecords where tableName='NewsPaper' and pkValue='".makeSafe($selected_id)."'");
	...
	if(function_exists('NewsPaper_before_update')){
		$args=array();
		if(!NewsPaper_before_update($data, getMemberInfo(), $args)){ return false; }
	}

	$o=array('silentErrors' => true);
	sql('update `NewsPaper` set       `Language`=' . (($data['Language'] !== '' && $data['Language'] !== NULL) ? "'{$data['Language']}'" : 'NULL') . ', `Name`=' . (($data['Name'] !== '' && $data['Name'] !== NULL) ? "'{$data['Name']}'" : 'NULL') . ', `Date_Of_Receipt`=' . (($data['Date_Of_Receipt'] !== '' && $data['Date_Of_Receipt'] !== NULL) ? "'{$data['Date_Of_Receipt']}'" : 'NULL') . ', `Date_Published`=' . (($data['Date_Published'] !== '' && $data['Date_Published'] !== NULL) ? "'{$data['Date_Published']}'" : 'NULL') . ', `Pages`=' . (($data['Pages'] !== '' && $data['Pages'] !== NULL) ? "'{$data['Pages']}'" : 'NULL') . ', `Price`=' . (($data['Price'] !== '' && $data['Price'] !== NULL) ? "'{$data['Price']}'" : 'NULL') . ', `Type`=' . (($data['Type'] !== '' && $data['Type'] !== NULL) ? "'{$data['Type']}'" : 'NULL') . ', `Publisher`=' . (($data['Publisher'] !== '' && $data['Publisher'] !== NULL) ? "'{$data['Publisher']}'" : 'NULL') . ', `Typess`=' . (($data['Typess'] !== '' && $data['Typess'] !== NULL) ? "'{$data['Typess']}'" : 'NULL') . ', `Pu`=' . (($data['Pu'] !== '' && $data['Pu'] !== NULL) ? "'{$data['Pu']}'" : 'NULL') ." where `id`='".makeSafe($selected_id)."'", $o);
	if($o['error']!=''){
		echo $o['error'];
		echo '<a href="NewsPaper_view.php?SelectedID='.urlencode($selected_id)."\">{$Translation['< back']}</a>";
		exit;
	}


	// hook: NewsPapers_after_update
	if(function_exists('NewsPaper_after_update')){
		$res = sql("SELECT * FROM `NewsPaper` WHERE `id`='{$data['selectedID']}' LIMIT 1", $eo);
		if($row = db_fetch_assoc($res)){
			$data = array_map('makeSafe', $row);
		}
		$data['selectedID'] = $data['id'];
		$args = array();
		if(!NewsPaper_after_update($data, getMemberInfo(), $args)){ return; }
	}

	// mm: update ownership data
	sql("update membership_userrecords set dateUpdated='".time()."' where tableName='NewsPaper' and pkValue='".makeSafe($selected_id)."'", $eo);

}

function NewsPaper_form($selected_id = '', $AllowUpdate = 1, $AllowInsert = 1, $AllowDelete = 1, $ShowCancel = 0, $TemplateDV = '', $TemplateDVP = ''){
	...
	if($selected_id){
		...
		$res = sql("select * from `NewsPaper` where `id`='".makeSafe($selected_id)."'", $eo);
		if(!($row = db_fetch_array($res))){
			return error_message($Translation['No records found'], 'NewsPaper_view.php', false);
		}
	...
	if($dvprint){
		$template_file = is_file("./{$TemplateDVP}") ? "./{$TemplateDVP}" : './templates/NewsPaper_templateDVP.html';
		$templateCode = @file_get_contents($template_file);
	}else{
		$template_file = is_file("./{$TemplateDV}") ? "./{$TemplateDV}" : './templates/NewsPaper_templateDV.html';
		$templateCode = @file_get_contents($template_file);
	}
	...
	// process buttons
	if($AllowInsert){
		if(!$selected_id) $templateCode = str_replace('<%%INSERT_BUTTON%%>', '<button type="submit" class="btn btn-success" id="insert" name="insert_x" value="1" onclick="return NewsPaper_validateData();"><i class="glyphicon glyphicon-plus-sign"></i> ' . $Translation['Save New'] . '</button>', $templateCode);
		$templateCode = str_replace('<%%INSERT_BUTTON%%>', '<button type="submit" class="btn btn-default" id="insert" name="insert_x" value="1" onclick="return NewsPaper_validateData();"><i class="glyphicon glyphicon-plus-sign"></i> ' . $Translation['Save As Copy'] . '</button>', $templateCode);
	}else{
	...
	if($selected_id){
		...
		if($AllowUpdate){
			$templateCode = str_replace('<%%UPDATE_BUTTON%%>', '<button type="submit" class="btn btn-success btn-lg" id="update" name="update_x" value="1" onclick="return NewsPaper_validateData();" title="' . html_attr($Translation['Save Changes']) . '"><i class="glyphicon glyphicon-ok"></i> ' . $Translation['Save Changes'] . '</button>', $templateCode);
		}else{
...
$templateCode = str_replace('<%%UPLOADFILE(Typess)%%>', '', $templateCode);
	$templateCode = str_replace('<%%UPLOADFILE(Pu)%%>', '', $templateCode);
	...
	if($selected_id){
		...
		if( $dvprint) $templateCode = str_replace('<%%VALUE(Typess)%%>', safe_html($urow['Typess']), $templateCode);
		if(!$dvprint) $templateCode = str_replace('<%%VALUE(Typess)%%>', html_attr($row['Typess']), $templateCode);
		$templateCode = str_replace('<%%URLVALUE(Typess)%%>', urlencode($urow['Typess']), $templateCode);
		if( $dvprint) $templateCode = str_replace('<%%VALUE(Pu)%%>', safe_html($urow['Pu']), $templateCode);
		if(!$dvprint) $templateCode = str_replace('<%%VALUE(Pu)%%>', html_attr($row['Pu']), $templateCode);
		$templateCode = str_replace('<%%URLVALUE(Pu)%%>', urlencode($urow['Pu']), $templateCode);
	}else{
		...
		$templateCode = str_replace('<%%VALUE(Typess)%%>', '', $templateCode);
		$templateCode = str_replace('<%%URLVALUE(Typess)%%>', urlencode(''), $templateCode);
		$templateCode = str_replace('<%%VALUE(Pu)%%>', '', $templateCode);
		$templateCode = str_replace('<%%URLVALUE(Pu)%%>', urlencode(''), $templateCode);
	}
	...
	$rdata = $jdata = get_defaults('NewsPaper');
	if($selected_id){
		$jdata = get_joined_record('NewsPaper', $selected_id);
		if($jdata === false) $jdata = get_defaults('NewsPaper');
		$rdata = $row;
	}
	$cache_data = array(
	...
	$templateCode .= loadView('NewsPaper-ajax-cache', $cache_data);

	// hook: NewsPapers_dv
	if(function_exists('NewsPaper_dv')){
		$args=array();
		NewsPaper_dv(($selected_id ? $selected_id : FALSE), getMemberInfo(), $templateCode, $args);
	}
