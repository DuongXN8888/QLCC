<?php
	...
	if($thisMD5==$prevMD5){
		...
		// set up tables
		setupTable('books', "create table if not exists `books` (   `id` INT unsigned not null auto_increment , primary key (`id`), `ISBN_NO` VARCHAR(100) , `Book_Title` VARCHAR(200) , `Author_Name` VARCHAR(100) , `Quantity` INT , `Purchase_Date` DATE , `Edition` VARCHAR(40) , `Price` DECIMAL(10,2) default '0.00' , `Pages` INT , `Publisher` VARCHAR(140) ) CHARSET utf8", $silent);
		...
		setupIndexes('Return_Book', array('Book_Number','Member'));
		 setupTable('Type', "create table if not exists `Type` (   `id` INT unsigned not null auto_increment , primary key (`id`), `Name` VARCHAR(40) ) CHARSET utf8", $silent);
		
setupTable('Types', "create table if not exists `Types` (   `id` INT unsigned not null auto_increment , primary key (`id`), `Name` VARCHAR(40) , `tt` VARCHAR(140) , `CC` VARCHAR(40) , `ID_N` VARCHAR(20) ) CHARSET utf8", $silent);
setupTable('Typ', "create table if not exists `Typ` (   `id` INT unsigned not null auto_increment , primary key (`id`), `Na` VARCHAR(40) , `t` VARCHAR(140) , `C` VARCHAR(40) , `D` VARCHAR(20), `D` VARCHAR(11)) CHARSET utf8", $silent);
setupTable('Return', "create table if not exists `Return` (   `id` INT unsigned not null auto_increment , primary key (`id`), `Book_Number` INT unsigned , `Book_Title` INT unsigned , `Issue_Date` INT unsigned default '1' , `Due_Date` INT unsigned default '1' , `Return_Date` DATE , `Member` INT unsigned , `Number` INT unsigned , `Fine` DECIMAL(10,2) default '0.0' , `Status` VARCHAR(40) ) CHARSET utf8", $silent);
		// save MD5
