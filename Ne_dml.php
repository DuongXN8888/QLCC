...
function Ne_insert(){
	...
	$arrPerm=getTablePermissions('Ne');
	if(!$arrPerm[1]){
		return false;
	}
	...
	if(function_exists('Ne_before_insert')){
		$args=array();
		if(!Ne_before_insert($data, getMemberInfo(), $args)){ return false; }
	}

	$o = array('silentErrors' => true);
	sql('insert into `Ne` set       `Language`=' . (($data['Language'] !== '' && $data['Language'] !== NULL) ? "'{$data['Language']}'" : 'NULL') . ', `Name`=' . (($data['Name'] !== '' && $data['Name'] !== NULL) ? "'{$data['Name']}'" : 'NULL') . ', `Date_Of_Receipt`=' . (($data['Date_Of_Receipt'] !== '' && $data['Date_Of_Receipt'] !== NULL) ? "'{$data['Date_Of_Receipt']}'" : 'NULL') . ', `Date_Published`=' . (($data['Date_Published'] !== '' && $data['Date_Published'] !== NULL) ? "'{$data['Date_Published']}'" : 'NULL') . ', `Pages`=' . (($data['Pages'] !== '' && $data['Pages'] !== NULL) ? "'{$data['Pages']}'" : 'NULL') . ', `Price`=' . (($data['Price'] !== '' && $data['Price'] !== NULL) ? "'{$data['Price']}'" : 'NULL') . ', `Type`=' . (($data['Type'] !== '' && $data['Type'] !== NULL) ? "'{$data['Type']}'" : 'NULL') . ', `Publisher`=' . (($data['Publisher'] !== '' && $data['Publisher'] !== NULL) ? "'{$data['Publisher']}'" : 'NULL'). ', `Typess`=' . (($data['Typess'] !== '' && $data['Typess'] !== NULL) ? "'{$data['Typess']}'" : 'NULL'). ', `Pu`=' . (($data['Pu'] !== '' && $data['Pu'] !== NULL) ? "'{$data['Pu']}'" : 'NULL'), $o);
	if($o['error']!=''){
		echo $o['error'];
		echo "<a href=\"Ne_view.php?addNew_x=1\">{$Translation['< back']}</a>";
		exit;
	}
	...
	if(function_exists('New_after_insert')){
		$res = sql("select * from `Ne` where `id`='" . makeSafe($recID, false) . "' limit 1", $eo);
	...
	set_record_owner('Ne', $recID, getLoggedMemberID());

	return $recID;
}

function Ne_delete($selected_id, $AllowDeleteOfParents=false, $skipChecks=false){
	...
	$arrPerm=getTablePermissions('Ne');
	$ownerGroupID=sqlValue("select groupID from membership_userrecords where tableName='Ne' and pkValue='$selected_id'");
	$ownerMemberID=sqlValue("select lcase(memberID) from membership_userrecords where tableName='Ne' and pkValue='$selected_id'");
	...
	if(function_exists('Ne_before_delete')){
		$args=array();
		if(!Ne_before_delete($selected_id, $skipChecks, getMemberInfo(), $args))
			return $Translation['Couldn\'t delete this record'];
	}

	sql("delete from `Ne` where `id`='$selected_id'", $eo);

	// hook: NewsPapers_after_delete
	if(function_exists('Ne_after_delete')){
		$args=array();
		Ne_after_delete($selected_id, getMemberInfo(), $args);
	}

	// mm: delete ownership data
	sql("delete from membership_userrecords where tableName='Ne' and pkValue='$selected_id'", $eo);
}

function Ne_update($selected_id){
	global $Translation;

	// mm: can member edit record?
	$arrPerm=getTablePermissions('Ne');
	$ownerGroupID=sqlValue("select groupID from membership_userrecords where tableName='Ne' and pkValue='".makeSafe($selected_id)."'");
	$ownerMemberID=sqlValue("select lcase(memberID) from membership_userrecords where tableName='Ne' and pkValue='".makeSafe($selected_id)."'");
	...
	if(function_exists('Ne_before_update')){
		$args=array();
		if(!Ne_before_update($data, getMemberInfo(), $args)){ return false; }
	}

	$o=array('silentErrors' => true);
	sql('update `Ne` set       `Language`=' . (($data['Language'] !== '' && $data['Language'] !== NULL) ? "'{$data['Language']}'" : 'NULL') . ', `Name`=' . (($data['Name'] !== '' && $data['Name'] !== NULL) ? "'{$data['Name']}'" : 'NULL') . ', `Date_Of_Receipt`=' . (($data['Date_Of_Receipt'] !== '' && $data['Date_Of_Receipt'] !== NULL) ? "'{$data['Date_Of_Receipt']}'" : 'NULL') . ', `Date_Published`=' . (($data['Date_Published'] !== '' && $data['Date_Published'] !== NULL) ? "'{$data['Date_Published']}'" : 'NULL') . ', `Pages`=' . (($data['Pages'] !== '' && $data['Pages'] !== NULL) ? "'{$data['Pages']}'" : 'NULL') . ', `Price`=' . (($data['Price'] !== '' && $data['Price'] !== NULL) ? "'{$data['Price']}'" : 'NULL') . ', `Type`=' . (($data['Type'] !== '' && $data['Type'] !== NULL) ? "'{$data['Type']}'" : 'NULL') . ', `Publisher`=' . (($data['Publisher'] !== '' && $data['Publisher'] !== NULL) ? "'{$data['Publisher']}'" : 'NULL') . ', `Typess`=' . (($data['Typess'] !== '' && $data['Typess'] !== NULL) ? "'{$data['Typess']}'" : 'NULL') . ', `Pu`=' . (($data['Pu'] !== '' && $data['Pu'] !== NULL) ? "'{$data['Pu']}'" : 'NULL') ." where `id`='".makeSafe($selected_id)."'", $o);
	if($o['error']!=''){
		echo $o['error'];
		echo '<a href="Ne_view.php?SelectedID='.urlencode($selected_id)."\">{$Translation['< back']}</a>";
		exit;
	}
	...
	if(function_exists('New_after_update')){
		$res = sql("SELECT * FROM `Ne` WHERE `id`='{$data['selectedID']}' LIMIT 1", $eo);
		if($row = db_fetch_assoc($res)){
			$data = array_map('makeSafe', $row);
		}
	...
	sql("update membership_userrecords set dateUpdated='".time()."' where tableName='Ne' and pkValue='".makeSafe($selected_id)."'", $eo);

}

function Ne_form($selected_id = '', $AllowUpdate = 1, $AllowInsert = 1, $AllowDelete = 1, $ShowCancel = 0, $TemplateDV = '', $TemplateDVP = ''){
	...
	if($selected_id){
		...
		$res = sql("select * from `Ne` where `id`='".makeSafe($selected_id)."'", $eo);
		if(!($row = db_fetch_array($res))){
			return error_message($Translation['No records found'], 'Ne_view.php', false);
		}
	...
	if($dvprint){
		$template_file = is_file("./{$TemplateDVP}") ? "./{$TemplateDVP}" : './templates/Ne_templateDVP.html';
		$templateCode = @file_get_contents($template_file);
	}else{
		$template_file = is_file("./{$TemplateDV}") ? "./{$TemplateDV}" : './templates/Ne_templateDV.html';
		$templateCode = @file_get_contents($template_file);
	}
...
$templateCode = str_replace('<%%UPLOADFILE(Typess)%%>', '', $templateCode);
	...
	$rdata = $jdata = get_defaults('Ne');
	if($selected_id){
		$jdata = get_joined_record('Ne', $selected_id);
		if($jdata === false) $jdata = get_defaults('Ne');
		$rdata = $row;
	}
	$cache_data = array(
	...
	// hook: NewsPapers_dv
	if(function_exists('Ne_dv')){
		$args=array();
		Ne_dv(($selected_id ? $selected_id : FALSE), getMemberInfo(), $templateCode, $args);
	}
